...
runApp = do 
  resp <- prepareAppReqs
  run 3000 $ either (error . show) app resp
...
prepareAppReqs =
  DB.initDB (dbFilePath firstAppConfig) >>= \resp -> 
    pure $ case resp of 
...
mkErrorResponse (DBError sqlError) =
  resp500 PlainText (LBS.pack sqlError)
